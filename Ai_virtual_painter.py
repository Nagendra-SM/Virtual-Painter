...
while True:
    ...
    lmList, bbox = detector.findPosition(img, draw=False)  # using function to find specific landmark position
    # draw false means no circles on landmarks
    if len(lmList) != 0:
        # print(lmList)
        ...
        if fingers[1] and fingers[2]:
            ...
            if y1 < 125:
                if 250 < x1 < 450:  # if i m clicking at purple brush
                    ...
                    drawColor = (0, 0, 0)
            cv2.rectangle(img, (x1, y1 - 25), (x2, y2 + 25), drawColor, cv2.FILLED)  # selection mode is represented
            # as rectangle
        ...
        if fingers[1] and fingers[2] == False:
            ...
            # print("Drawing Mode")
            if xp == 0 and yp == 0:  # initially xp and yp will be at 0,0 so it will draw a line from 0,
                # 0 to whichever point our tip is at
                xp, yp = x1, y1  # so to avoid that we set xp=x1 and yp=y1
            # till now we are creating our drawing but it gets removed as everytime our frames are updating so we
            # have to define our canvas where we can draw and show also
    ...
    _, imgInv = cv2.threshold(imgGray, 50, 255,
                              cv2.THRESH_BINARY_INV)  # on canvas all the region in which we drew is black and where
    # it is black it is cosidered as white,it will create a mask
    ...
    img[0:150, 0:1280] = header  # on our frame we are setting our JPG image acc to H,W of jpg images
